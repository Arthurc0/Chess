<template>
    <form @submit.prevent="submitForm">
        <div class="flex flex-col gap-4">
            <slot :setFormValue="setFormValue" :errors="errors"></slot>
        </div>
        <button type="submit" class="inline-flex gap-3 self-center items-center w-fit font-semibold tracking-wide bg-teal-800 px-10 py-3 rounded-3xl text-sm text-stone-100 hover:bg-teal-900 disabled:opacity-75 transition duration-[200ms] outline-none border-[3px] border-transparent focus:border-green-600" :disabled="pending">
            <AppLoader v-if="pending" size="22" />
...
const setFormValue = async (names: string[], event: Event): Promise<void> => {
